...
@section('content')
    <div class="mb-8 border-b border-[#E5E3DB] pb-5">
        ...
        <h1 class="font-display mt-2 text-3xl font-semibold text-[#16213A]">Ubah Jurusan</h1>
        <p class="mt-1 text-sm text-slate-500">Memperbarui data untuk jurusan <span class="font-medium text-[#16213A]">{{ $major['name'] }}</span>.</p>
    </div>

    <form action="{{ route('majors.update', ['major' => $major['id']]) }}" method="POST" class="space-y-6 border border-[#E5E3DB] bg-white p-8">
        @csrf
        @method('PUT')
        ...
        <div>
            <label for="description" class="mb-1.5 block text-xs font-semibold uppercase tracking-[0.1em] text-[#16213A]">Keterangan</label>
