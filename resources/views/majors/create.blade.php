...
@section('content')
    <div class="mb-8 border-b border-[#E5E3DB] pb-5">
        ...
        <p class="mt-1 text-sm text-slate-500">Isi data untuk menambahkan jurusan baru ke sistem.</p>
    </div>
    ...
    <form action="{{ route('majors.store') }}" method="POST" class="space-y-6 border border-[#E5E3DB] bg-white p-8">
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="4"
                class="w-full border border-[#D9D6CD] bg-[#FCFBF8] px-3.5 py-2.5 text-sm placeholder:text-slate-400 focus:border-[#A16207] focus:bg-white focus:outline-none"
                placeholder="Deskripsi singkat jurusan"></textarea>
        </div>

        <div class="flex justify-end gap-4 border-t border-[#EFEDE6] pt-6">
            <a href="{{ route('majors.index') }}" class="px-4 py-2.5 text-sm font-medium text-slate-500 hover:text-[#16213A]">Batal</a>
            <button type="submit"
                class="bg-[#16213A] px-6 py-2.5 text-sm font-medium text-white transition hover:bg-[#26324f]">Simpan Jurusan</button>
        </div>
    </form>
@endsection
