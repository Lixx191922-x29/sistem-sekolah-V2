    ...
    <div class="mt-3 border border-[#E5E3DB] bg-white">
        <div class="flex items-start justify-between border-b border-[#E5E3DB] bg-[#FCFBF8] px-8 py-6">
            <div>
                ...
                <p class="mt-1 font-mono text-xs text-slate-500">{{ $teacher['subject'] }}</p>
            </div>
        ...
        <dl class="divide-y divide-[#EFEDE6] text-sm">
            ...
            <div class="flex justify-between px-8 py-4">
                <dt class="uppercase tracking-[0.1em] text-xs text-slate-400">Nama Guru</dt>
        ...
        <div class="flex justify-end gap-4 border-t border-[#E5E3DB] px-8 py-5">
            ...
            <form action="{{ route('teachers.destroy', ['id' => $teacher['id']]) }}" method="POST" onsubmit="return confirm('Hapus data guru ini?')">
                @csrf
                @method('DELETE')
                <button type="submit"
