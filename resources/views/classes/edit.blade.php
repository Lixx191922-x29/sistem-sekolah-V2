...
@section('content')
    <div class="mb-8 border-b border-[#E5E3DB] pb-5">
        ...
        <h1 class="font-display mt-2 text-3xl font-semibold text-[#16213A]">Ubah Kelas</h1>
        <p class="mt-1 text-sm text-slate-500">Memperbarui data untuk kelas <span class="font-medium text-[#16213A]">{{ $class['name'] }}</span>.</p>
    </div>
        ...
        <div>
            ...
            <select id="major_id" name="major_id"
                ...
                @foreach ($majors as $major)
                    <option value="{{ $major['id'] }}" {{ ($class['major_id'] ?? '') == $major['id'] ? 'selected' : '' }}>{{ $major['code'] }} - {{ $major['name'] }}</option>
                @endforeach
            </select>
        </div>
